  ...
  test('User to validate that the message error will be gone after populating the mandatory fields', async () => {
    ...
    await test.step('User should be able to see the validation errors', async () => {
      ...
      await commonUtils.assertElement([
        reportFeedback.lblForenameError,
        reportFeedback.lblEmailError,
        reportFeedback.lblMessageError,
      ]);
      await commonUtils.takeScreenshot(
        `reportFeeback - ${dateToday}`,
        `User should be able to see the validation errors`,
      );
    });

    await test.step('User have populated all fields', async () => {
      const reportFeedback = new ReportFeedback(page);
      await reportFeedback.populateContactFields(
        UserInfo.userOne.fName,
        UserInfo.userOne.lName,
        UserInfo.userOne.emailAdd,
        UserInfo.userOne.tPNumber,
        UserInfo.userOne.message,
      );
    });
    ...
    await test.step('Validate that all of the message errors are gone', async () => {
      const reportFeedback = new ReportFeedback(page);
      const commonUtils = new CommonUtils(page);
      await commonUtils.assertElementVisibility(
        [
          reportFeedback.lblForenameError,
          reportFeedback.lblEmailError,
          reportFeedback.lblMessageError,
      ...
      await commonUtils.takeScreenshot(
        `reportFeeback - ${dateToday}`,
        `Validate that all of the message errors are gone`,
      );
    });
  });
...
*/
  ...
  test('User should be able to succesfully submit a message', async () => {
    ...
    await test.step('User should be able to successfully submit a feedback', async () => {
      ...
      console.log('TEST dateToday ' + dateToday);
      await commonUtils.takeScreenshot(
        `reportFeeback - ${dateToday}`,
        `User should be able to successfully submit a feedback`,
      );
    });
  });
